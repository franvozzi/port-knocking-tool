...
class ImportTest:
    ...
    def test_import(self, module_name: str, imports: List[str]) -> bool:
        ...
        try:
            ...
            self.results.append(
                {"module": module_name, "imports": imports, "error": str(e), "success": False}
            )
            return False
    ...
    def run_test(self, test_name: str, module_name: str, imports: List[str]) -> None:
        ...
        if self.test_import(module_name, imports):
            self.passed += 1
            self.results.append({"module": module_name, "imports": imports, "success": True})
...
def main():
    ...
    tests = [
        ...
        ("Utils - Validators", "utils.validators", ["ConfigValidator"]),
        ("Utils - Constants", "utils.constants", ["VERSION", "COLORS", "STATUS_MESSAGES"]),
        ("Core - ConfigManager", "core.config_manager", ["ConfigManager"]),
        ("Core - PortKnocker", "core.port_knocker", ["PortKnocker"]),
        ("Core - VPNManager", "core.vpn_manager", ["VPNManager", "get_vpn_manager"]),
        ("Security - Crypto", "security.crypto", ["CredentialsEncryptor"]),
        ("Network - Diagnostics", "network.diagnostics", ["NetworkDiagnostics"]),
        ("Network - CircuitBreaker", "network.circuit_breaker", ["CircuitBreaker", "CircuitState"]),
        ("Monitoring - Logger", "monitoring.logger", ["StructuredLogger"]),
        ("Monitoring - Metrics", "monitoring.metrics", ["MetricsCollector"]),
        ("UI - Widgets StatusBar", "ui.widgets.status_bar", ["StatusBar"]),
        ("UI - Widgets ProgressBar", "ui.widgets.progress_bar", ["ConnectionProgress"]),
        ("UI - Themes", "ui.themes", ["ThemeManager", "THEMES"]),
        ("UI - Main GUI", "ui.gui_main", ["VPNConnectGUI"]),

...
def test_linux_connect_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "src.core.vpn_manager.subprocess.run", lambda *a, **k: make_subprocess_return(0)
    )
...
def test_linux_connect_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "src.core.vpn_manager.subprocess.run",
        lambda *a, **k: make_subprocess_return(1, stderr="error"),
    )
...
def test_linux_disconnect_and_is_connected(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.core.vpn_manager.subprocess.run", lambda *a, **k: make_subprocess_return(1)
    )

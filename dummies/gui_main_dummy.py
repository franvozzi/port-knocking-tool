...
class VPNConnectDummyGUI:
    def __init__(self, simulate_success=True):
        ...
        self.status_label = tk.Label(self.window, text="Listo para conectar (dummy)", font=("Arial", 11), fg="#666")
        self.status_label.pack(pady=20)
        
        self.connect_btn = tk.Button(self.window, text="Conectar", command=self.start_connection,
                                     font=("Arial", 13, "bold"), bg="#4CAF50", fg="white",
                                     width=18, height=2, cursor="hand2")
        self.connect_btn.pack(pady=15)
        
        self.disconnect_btn = tk.Button(self.window, text="Desconectar", command=self.disconnect_vpn,
                                        font=("Arial", 10), state="disabled", width=18)
    ...
    def do_connection(self):
        try:
            ...
            if not success:
                ...
                messagebox.showerror("Error de Conexión", "Dummy: Port knocking fallido (puerto cerrado).")
